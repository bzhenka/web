export default class Canvas{
    ...
    drawResult(response) {
        let [dumb_x, dumb_y] = this.normal_to_dumb(response.x, response.y);
        ...
        ctx.fill();
        this.clickedPoints.push(response);
        this.drawSavedPoints();
    }
    ...
    drawSavedPoints() {
        ...
        for (const response of this.clickedPoints) {
            ctx.fillStyle = response.hit ? "green" : "red";
            ctx.beginPath();
            ctx.arc((response.x / response.radius) * this.radius, (response.y / response.radius) * this.radius, (1/100) * this.width, 0, Math.PI * 2);
            ctx.fill();
        }
    }
